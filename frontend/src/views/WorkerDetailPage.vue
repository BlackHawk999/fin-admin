<template>
  <div class="worker-detail-page" data-testid="worker-detail-page">
    ...
    <div
      v-if="loading"
      class="worker-detail-page__loading"
    ...
    <template v-else-if="employee">
      ...
      <!-- DateRangePicker может не пробрасывать attrs => обертка -->
      <div
        class="worker-detail-page__daterange"
        data-testid="worker-advances-daterange"
      >
        <DateRangePicker v-model="dateRange" />
      </div>

      <div
        class="worker-detail-page__table-wrap"
        data-testid="worker-advances-wrap"
      >
          ...
          >
            ...
            <template #actions="{ row }">
              <button
                ...
                @click="deleteAdv(row)"
                :data-testid="`advance-delete-${row.id}`"
              >
                O'chirish
              </button>
            </template>
      ...
      <BaseModal
        ...
        title="Yangi avans"
        @update:model-value="showAdvanceModal = false"
        data-testid="advance-modal"
      >
        <form
          class="form"
          @submit.prevent="addAdvance"
        ...
        >
          ...
          />
          <MoneyInput
            v-model="advanceForm.amount_uzs"
            label="Summa"
            data-testid="advance-amount"
          />
          <BaseInput
            v-model="advanceForm.comment"
            label="Izoh"
            data-testid="advance-comment"
          />
        </form>
...
const summaryCards = computed(() => {
  ...
  return [
    ...
    { title: "Авансы за период", value: formatUzs(advancesSum.value) },
    {
      ...
      value: formatUzs(remainder > 0 ? remainder : 0),
    },
  ];
});

const advancesSummary = computed(
  () => `Jami avanslar: ${formatUzs(advancesSum.value)}`,
);
